...
```bash
pip install pygame3d_generic
```

GPU アクセラレータ付き（任意）:

```bash
pip install "pygame3d_generic[accelerate]"
```

開発用:

```bash
pip install "pygame3d_generic[dev]"
```
...
```python
from pygame3d_generic.core import DynamicEntity, PhysicsBody, Camera
...
```python
from pygame3d_generic.core import Projectile
...
```python
from pygame3d_generic.core import DynamicEntity
...
```python
from pygame3d_generic.world import generate_grid_world, BoxVolume, Road
...
```python
from pygame3d_generic.render import (
    setup_lighting,
...
```python
from pygame3d_generic.network import NetworkServer, NetworkClient
